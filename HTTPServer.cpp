...
void HTTPServer::SetGameState(Scale *myScale, FieldController *fieldController) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          if (strncasecmp("state=", command, 6) == 0)
          {
            if (strncasecmp("off", &command[6], 3) == 0) {
              ...
              fieldController->initializeScaleSettings(myScale->getFieldLeftAlliance(), myScale->getLHSAlliance(), myScale->getRHSAlliance(), Ethernet.localIP());
            }
          }
          Serial.println();
