 ...
 * Field left alliance color pin 7 (high, un-jumpered is RED)
...
#define LHSLEDPIN 5
#define RHSLEDPIN 6
#define FLALLIANCEPIN 7  // Alliance color field left (high, unjumpered = RED)
...
AllianceSelector allianceSelector(FLALLIANCEPIN);

// Define a representation of the physical switch
Scale theScale(LHSSWITCHPIN, RHSSWITCHPIN, LHSLEDPIN, RHSLEDPIN);
...
// Define a representation of the field controller so we can send updates when the scale changes
...
void setup() {
  ...
  // Tell the scale class which field left alliance we are
  theScale.setFieldLeftAlliance(allianceSelector.Is());
  Serial.print(F("Field Left Alliance is "));
  ...
  if (connected)
  {
    ...
    fieldController.initializeScaleSettings(theScale.getFieldLeftAlliance(), theScale.getLHSAlliance(), theScale.getRHSAlliance(), Ethernet.localIP());
  }
  theScale.off();
}
...
void loop() {
  ...
  if (connected) {
    httpServer.SetGameState(&theScale, &fieldController);
  }
  
  // Get the switch state
  theScale.updateState();
  theScale.updatePlates();

  // If the state of the switch has changed, 
  if (theScale.stateChanged() && theScale.getGameState() == GameState::PLAY)
  {
    if (connected) {
      Serial.println(F("Posting change to server..."));
      // Send the update to the field controller
      fieldController.updateScale(theScale.isLHSOwned(), theScale.isRHSOwned());
    }
  }
  theScale.acknowledgeStateChanged();
}
